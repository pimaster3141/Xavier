...
class DataProcessorDCS(mp.Process):
# class DataProcessorDCS(threading.Thread):
	QUEUE_TIMEOUT = 1;
	QUEUE_DEPTH = 100;
	G2_LEVELS = 6;
	...
	def __init__(self, MPI, inputBuffer, averages, legacy, fs, bufferSize, sampleSize=2, packetMultiple=4, calcFlow=False, SNRBufferDepth=500, numProcessors=None):
		...
		self.tauList = XavierG2Calc.mtAuto(np.ones(self.packetSize*self.packetMultiple-1), fs=fs, levels=DataProcessorDCS.G2_LEVELS)[:,0];
		

		self.snrBuffer = np.ones((len(self.averages), SNRBufferDepth, len(self.tauList)-1));
		self.snrBuffer[0,:,:] = 0;

		self.g2Buffer = mp.Queue(DataProcessorDCS.QUEUE_DEPTH);
		# self.countBuffer = mp.Queue(DataProcessorDCS.QUEUE_DEPTH);
		self.flowBuffer = mp.Queue(DataProcessorDCS.QUEUE_DEPTH);
	...
	def run(self):
		...
		try:
			...
			g2Fcn = partial(XavierG2Calc.calculateG2, fs=self.fs, levels=DataProcessorDCS.G2_LEVELS, legacy=self.legacy);
			while(not self.isDead.is_set()):				
				try:
					for i in range(self.packetMultiple):
						initialData[i*self.packetSize:(i+1)*self.packetSize] = self.inputBuffer.get(block=True, timeout=DataProcessorDCS.QUEUE_TIMEOUT);
				except queue.Empty:					
					continue
				...
				try:
					for i in range(inWaiting):
						for j in range(self.packetMultiple):
							data[i+1][j*self.packetSize:(j+1)*self.packetSize] = self.inputBuffer.get(block=True, timeout=DataProcessorDCS.QUEUE_TIMEOUT);
				except queue.Empty:
					print("FUCK");
					pass
...
# class DataProcessorDCSNIRS(mp.Process):
# # class DataProcessorDCS(threading.Thread):
# 	QUEUE_TIMEOUT = 1;
# 	QUEUE_DEPTH = 100;
# 	G2_LEVELS = 6;
